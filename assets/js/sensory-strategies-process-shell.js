/* Keep process-page content outside the shell rebuilt by language selection. */
(() => {
  "use strict";

  const start = () => {
    const shell = document.getElementById("site-shell");
    const main = document.querySelector("main#main.spc-main[data-home-detail]");
    ...
    const sync = () => {
      ...
      for (const container of [shell, footer]) {
        container.querySelectorAll('.site-header a[href^="#"], .mobile-nav a[href^="#"], .site-footer a[href^="#"], #searchModal a[href^="#"]').forEach((link) => {
